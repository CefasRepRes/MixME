...
getC <- function(object, x, sl, summarise = TRUE, na.rm = FALSE) {
  ...
  res <- lapply(seq_along(object), function(ii) {
    ...
    if (is.na(stki)) {
      ...
      if (sl %in% c("landings.n","discards.n")) {
        return(apply(slot(i[[x]], sl), c(1:4,6), sum, na.rm=na.rm))
      }
      if (sl == "landings.wt") {
        LF <- sweep(slot(i[[x]], "landings.n"),c(1:4,6),apply(slot(i[[x]],"landings.n"),c(1:4,6),sum, na.rm=na.rm),"/")
        LF[is.na(LF)] <- 0
        return(apply(slot(i[[x]],"landings.wt")*LF,c(1:4,6),sum, na.rm=na.rm))
      }
      if (sl == "discards.wt") {
        DF <- sweep(slot(i[[x]], "discards.n"),c(1:4,6),apply(slot(i[[x]],"discards.n"),c(1:4,6),sum, na.rm=na.rm),"/")
        DF[is.na(DF)] <- 0
        return(apply(slot(i[[x]],"discards.wt")*DF,c(1:4,6),sum, na.rm=na.rm))
      }
    }
  })
  ## Assign fleet names
...
getCW <- function(object, x, sl = "landings", summarise = TRUE, na.rm=TRUE) {
