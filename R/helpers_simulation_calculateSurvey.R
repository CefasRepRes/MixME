...
calcSurveyIndex <- function(stk, flt, idx, use_q = TRUE, use_time = TRUE, use_fastF = TRUE) {
  ...
  # Fa <- getFage(stk, flt, stkname, yr = years, use_fastF = TRUE)[ac(ages), ac(years),,,, ac(iter)]
  
  if(use_fastF) {

    arr <- array(0,
                 dim = c(dim(FLCore::n(stk)),length(flt)),
                 dimnames = c(dimnames(FLCore::n(stk)), list(flt = names(flt))))
    pFa <- fa_cpp(arr = arr, flts = flt, stockname = stkname)
    Fa <- index.n
    Fa[] <- apply(pFa, 1:6, sum)[ac(ages), ac(years),,,, ac(iter)]
