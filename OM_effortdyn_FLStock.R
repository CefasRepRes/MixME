...
#'
#' This function converts the linked \code{FLStocks} and \code{FLFisheries}
...
#'     }
#'     \item \code{wt}: a named list
#'     \itemize{
#'       \item vector of individual mean weight-at-age for ith stock
#'     }
...
#'       SHOULD I CHANGE THE ASSUMPTION THAT FLSTOCK IS USED?
#'
#' @export

FLStocks2List <- function(om, year, advice, adviceType = "catch") {
  ...
  omList <- lapply(1:ni, function(x){
    ...
    n_age <- lapply(1:nstk, function(y){
      stock.n(iter(om$stks[[y]],x))[,ac(year), drop = TRUE]
    })
    ...
    ## extract individual mean weight-at-age
    ## (This should be landings weight or catch weight depending on advice basis)
    if(adviceType == "catch"){
      wt_age <- lapply(1:nstk, function(y){
        wt_y <- sapply(1:nflt, function(z){
          catch.wt(iter(om$flts[[z]][[y]], x))[,ac(year), drop = TRUE]
        })
        colnames(wt_y) <- names(om$flts)
        return(wt_y)
      })
      names(wt_age) <- names(om$stks)
    } else if(adviceType == "landings") {
      wt_age <- lapply(1:nstk, function(y){
        wt_y <- sapply(1:nflt, function(z){
      ...
      stop("adviceType must be 'catch' or 'landings'")
    }
...
effortBaranov <- function(omList,
  ...
  Gobj <- function(par, dat, adviceType) {
    ...
    Cfleet <- sapply(1:nrow(dat$partF),
                       function(x){
                         ...
                         ## Calculate Catch in weight at age per fleet
                         partCWage <- partCage * dat$wt[[x]]
  ...
  if(method == "nlminb") {
    ...
    eff <- nlminb(start = par, objective = Gobj, dat = omList[[1]],
  ...
  stkEff <- omList[[1]]$quota - BaranovCatch(par = exp(eff$par),
                                             dat = omList[[1]],
                                             adviceType = adviceType)
  ...
  if(method == "nlminb") {
    ...
    out <- nlminb(start = eff$par, objective = Fobj,
                  dat = omList[[1]], adviceType = adviceType,
                  control = list("iter.max" = 1000,
                                 "eval.max" = 1000))
  ...
  stkEff <- omList[[1]]$quota - BaranovCatch(par = exp(out$par),
                                             dat = omList[[1]],
                                             adviceType = adviceType)

  ## Check for mismatch in choke stocks
  stkLimMismatch <- !all(omList[[1]]$stkLim == sapply(1:ncol(stkEff), function(x) { which.min(stkEff[,x])}))
  ...
  if(stkLimMismatch) {
    ...
    omList[[1]]$stkLim <- sapply(1:ncol(stkEff), function(x) { which.min(stkEff[,x])})
  ...
  if(min(stkEff, na.rm =  TRUE) < -0.001 & correctResid == TRUE) {
    ...
    effmult <- optimise(function(x) {
      abs(min(omList[[1]]$quota - BaranovCatch(par = exp(out$par)*x,
                                               dat = omList[[1]],
                                               adviceType = adviceType),
              na.rm = TRUE))},
      c(0,1))
